...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_service'])) {
    ...
    if ($id) {
        ...
        $stmt->execute($data);
        echo '<div class="alert alert-success">सेवा अपडेट गरियो</div>';
    } else {
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start">
  <div class="form-card">
    ...
    <form method="POST">
      ...
      <?php endif; ?>
      <div class="form-grid">
        <div class="field"><label>शीर्षक (नेपाली) *</label><input name="title_ne" required value="<?= htmlspecialchars($editService['title_ne'] ?? '') ?>"></div>
        <div class="field"><label>शीर्षक (अङ्ग्रेजी)</label><input name="title_en" value="<?= htmlspecialchars($editService['title_en'] ?? '') ?>"></div>
        <div class="field"><label>विवरण (नेपाली)</label><textarea name="description_ne" rows="3"><?= htmlspecialchars($editService['description_ne'] ?? '') ?></textarea></div>
        <div class="field"><label>विवरण (अङ्ग्रेजी)</label><textarea name="description_en" rows="3"><?= htmlspecialchars($editService['description_en'] ?? '') ?></textarea></div>
        ...
        <div class="field"><label>मूल्य (रु)</label><input name="base_price" type="number" step="0.01" value="<?= htmlspecialchars($editService['base_price'] ?? '') ?>"></div>
        <div class="field"><label>अवधि (मिनेट)</label><input name="duration_minutes" type="number" value="<?= htmlspecialchars($editService['duration_minutes'] ?? '') ?>"></div>
  ...
  <div class="admin-table-wrapper">
    <table class="admin-table">
      ...
      <tbody>
        ...
        <tr>
          <td><strong><?= htmlspecialchars($s['title_ne']) ?></strong><?= $s['title_en'] ? '<br><small style="color:var(--muted)">' . htmlspecialchars($s['title_en']) . '</small>' : '' ?></td>
          <td><?= $s['category'] ?></td>
          <td><?= $s['base_price'] ? 'रु ' . number_format($s['base_price']) : '—' ?></td>
        ...
        <tr><td colspan="5" style="text-align:center;padding:32px;color:var(--muted)">कुनै सेवा छैन</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
