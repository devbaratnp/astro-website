...
$definitions = [
    ...
    'articles' => ['table'=>'articles','fields'=>['title_ne','title_en','slug','content_ne','content_en','excerpt_ne','excerpt_en','cover_image'],'required'=>['title_ne','content_ne']],
...
if (in_array($_SERVER['REQUEST_METHOD'], ['POST','PUT'], true)) {
    ...
    if ($resource === 'articles') {
        ...
        if (empty($values['slug'])) $values['slug'] = generateSlug($values['title_ne'] ?? '');
    }
    if ($resource === 'events' && !array_key_exists('is_active', $input)) {
        $values['is_active'] = 1;
    }
